...
public class PersonaInfoFragment extends ToolbarFragment {
    ...
    private Observable<Long> getCountObservable() {
        return SecureItemBll
                ...
                .flatMap(secureItemBll -> {
                    try {
                        return Observable.just(secureItemBll.getCountOfSecureItemsByType(ItemType.PersonalInfo));
    ...
    public void onEvent(SecureItemsRefreshEvent event) {
        ...
        if (!(fragment instanceof PersonaInfoSplashFragment)) {
            return;
        }
        getCountObservable()
                .subscribe(count -> {
                    ...
                    getChildFragmentManager()
                            .beginTransaction()
                            .replace(R.id.fr_fr_fragment, SecureItemsTabsFragment.newInstance(ItemType.PersonalInfo))
                            .commit();
                });
        ...
        if (null == savedInstanceState) {
            getCountObservable()
                    .subscribe(count -> {
                        Fragment fragment;
                        if (count > 0) {
                            fragment = SecureItemsTabsFragment.newInstance(ItemType.PersonalInfo);
                        } else {
                            fragment = new PersonaInfoSplashFragment();
                        }
                        getChildFragmentManager()
                                .beginTransaction()
