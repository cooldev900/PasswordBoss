...
public class DigitalWalletFragment extends ToolbarFragment {
    ...
    private Observable<Long> getCountObservable() {
        return SecureItemBll
                ...
                .flatMap(secureItemBll -> {
                    try {
                        return Observable.just(secureItemBll.getCountOfSecureItemsByType(ItemType.DigitalWallet));
    ...
    protected void invalidateToolbar(@NonNull AppToolbar toolbar) {
        ...
        toolbar.setTitle(R.string.DigitalWallet);
        if (isPhone()) {
    ...
    public void onEvent(SecureItemsRefreshEvent event) {
        ...
        if (!(fragment instanceof DigitalWalletSplashFragment)) {
            return;
        }
        getCountObservable()
                .subscribe(count -> {
                    ...
                    getChildFragmentManager()
                            .beginTransaction()
                            .replace(R.id.fr_fr_fragment, SecureItemsTabsFragment.newInstance(ItemType.DigitalWallet))
                            .commit();
                });
        ...
        if (null == savedInstanceState) {
            getCountObservable()
                    .subscribe(count -> {
                        Fragment fragment;
                        if (count > 0) {
                            fragment = SecureItemsTabsFragment.newInstance(ItemType.DigitalWallet);
                        } else {
                            fragment = new DigitalWalletSplashFragment();
                        }
                        getChildFragmentManager()
                                .beginTransaction()
